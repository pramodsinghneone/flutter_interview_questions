...
Ans - Yes we can use but optionally we don’t have to use because SeState will rebuild whole widget unnecessary that is very expensive to use. BlocBuilder will only rebuild the particular widget on which it is using.
...
Ans - 
- BlocObserver is a abstract class for monitoring BloC instances behavior. This means we can track anytime an event is triggered or a state is emitted.
